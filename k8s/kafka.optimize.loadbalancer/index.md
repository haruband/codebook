...
위의 그림은 클라이언트가 노드포트를 이용하여 서비스에 접근하는 과정이다. 클라이언트는 해당 서비스의 엔드포인트(Pod3)가 어느 노드에 있는지 알 수 없기 때문에 임의의 노드(위에서는 Node0)로 접속한다. 위의 경우에는 엔드포인트가 Node1 에 있었기 때문에, Node0 에서 SNAT 를 이용하여 Node1 로 요청 메세지를 송신하고 응답 메세지를 수신한 다음 클라이언트에게 전달한다. 이는 클라이언트가 처음부터 엔드포인트가 있는 노드(Node1)로 바로 접속했으면 발생하지 않았을 문제이기 때문에, 쿠버네티스가 제공하는 로드밸런서를 이용하면 간단히 해결할 수 있다. (즉, 부트스트랩이 전달해준 브로커의 주소가 노드포트 주소이기 때문에 실제 브로커 파드가 위치한 노드의 주소가 아닐 수 있다.)

지금부터 MetalLB 를 이용하여 어떻게 이 문제를 해결하였는지 살펴보도록 하자. MetalLB 는 L2 모드와 BGP 모드를 지원하기 때문에 각각 어떻게 동작하는지, 어떤 장단점이 있는지 하나씩 분석해보도록 하겠다. 다음은 간단히 실험에 사용된 노드의 주소(IP/MAC)이다.

- Node0 : 192.168.200.200 (f4:6b:8c:82:42:33)
- Node1 : 192.168.200.201 (f4:6b:8c:82:42:2b)
- Node2 : 192.168.200.202 (f4:6b:8c:82:41:f8)

아래는 부트스트랩과 브로커의 서비스 목록이다. 모두 로드밸런서로 설정되어 있으며, 할당된 주소를 확인할 수 있다. (해당 리스너의 Advertised 주소는 아래 로드밸런서 주소로 설정되어있다.)
...
그리고 브로커의 파드 목록이다. 각각 어느 노드에 배치되어 있는지 확인할 수 있다.
...
아래는 0번 브로커에 있는 토픽/파티션에 접근하는 클라이언트의 접속 정보이다. 위에 설정된 0번 브로커의 로드밸런서 주소(192.168.200.241)로 접속하는걸 볼 수 있다.
...
마지막으로 Node1 로 들어온 접속 요청을 어떻게 0번 브로커 파드로 전달하는지 살펴보자. 아래는 Node1 의 Cilium 서비스 목록이다. 로드밸런서로 들어온 요청을 0번 브로커 주소(10.0.2.233)으로 전달하는 것을 확인할 수 있다.
...
참고로 Node0 의 Cilium 서비스 목록을 살펴보면, 0번 브로커의 로드밸런서 주소에 대한 백엔드가 비어있는 것을 볼 수 있다. 이는 앞에서 외부 트래픽 정책을 로컬로 설정했기 때문에 다른 노드에 있는 파드는 백엔드 목록에서 제거되기 때문이다.
...
간단히 오픈소스 라우팅 데몬인 [BIRD](https://bird.network.cz/) 를 이용하여 MetalLB 의 BGP 모드를 검증하였다. 아래는 외부 트래픽 정책을 로컬로 설정했을 때 BIRD 가 설치된 노드의 라우팅 테이블이다. 브로커의 로드밸런서 주소는 각각 브로커 파드가 동작 중인 노드의 주소로 설정되어 있고, 부트스트랩의 로드밸런서 주소는 모든 브로커 파드가 동작 중인 세 개 노드의 주소가 설정되어 있다.
...
아래는 외부 트래픽 정책을 클러스터로 설정했을 때 BIRD 가 설치된 노드의 라우팅 테이블이다. 모든 노드를 통해서 모든 파드로 접속할 수 있기 때문에 모든 로드밸런서 주소는 모든 노드의 주소로 설정되어 있는 것을 볼 수 있다.
