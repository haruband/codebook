...
엔보이에서 가장 중요한 것 중의 하나는 필요한 트래픽을 리스너로 전달하는 것인데, 기존의 사이드카 방식의 프록시는 IPTables 를 이용하여 모든 트래픽을 엔보이의 특정 리스너로 전달하는 방식을 사용하지만, Cilium 의 노드별 프록시는 특정 서비스(cilium-ingress-basic-ingress)로 들어오는 트래픽을 특정 리스너(cilium-ingress-default-basic-ingress)로 전달하는 방식을 사용하고 있다. 지금부터 Cilium 이 어떤 방식으로 서비스의 트래픽을 리스너로 전달하는지 자세히 살펴보도록 하자.
